...
select
  ...
  concat(asmt.asmt_shrt_name, '-', asmt_sbst_rfrnc.asmt_sbst_shrt_name, '-', asmt_vrsn.asmt_vrsn_id) as key,
...
-- section loinc codes (based on assessment and subset)
select
  'section' as type,
  concat(
    asmt.asmt_shrt_name, '-', asmt_sbst_rfrnc.asmt_sbst_shrt_name, '-', asmt_vrsn.asmt_vrsn_id,
    ' / ', hit_sbst_sect_rlp.asmt_sect_id
  ) as key,
...
-- question loinc codes (based on assessment and subset)
select
  'question' as type,
  concat(
    asmt.asmt_shrt_name, '-', asmt_sbst_rfrnc.asmt_sbst_shrt_name, '-', asmt_vrsn.asmt_vrsn_id,
    ' / ', data_ele_qstn.data_ele_qstn_id
  ) as key,
...
-- response loinc codes (based on assessment and subset)
select
  'response' as type,
  concat(
    asmt.asmt_shrt_name, '-', asmt_sbst_rfrnc.asmt_sbst_shrt_name, '-', asmt_vrsn.asmt_vrsn_id,
    ' / ', data_ele_qstn.data_ele_qstn_id,
    ' / ', hit_rspns_val_map.data_ele_rspns_id,
    ' / ', hit_rspns_val_map.data_ele_rspns_val_id
  ) as key,
